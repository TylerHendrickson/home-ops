...
locals {
  ...
  main_docker_image = "ghcr.io/tortitude/coder-templates:cpf-reporter-feature-development"

  postgres_user           = "postgres"
  postgres_password       = "password123"
  postgres_dev_dbname     = "cpf_reporter"
  postgres_test_dbname    = "${local.postgres_dev_dbname}_test"
...
resource "coder_agent" "coder" {
  ...
  metadata {
    display_name = "Website Available?"
    key          = "3_website_available"
    script       = "ss -HOtln | grep -q ':8910' && echo yes || echo no"
    interval     = 30
    timeout      = 10
  }

  metadata {
    display_name = "API Available?"
    key          = "4_api_available"
    script       = "ss -HOtln | grep -q ':8911' && echo yes || echo no"
    interval     = 30
    timeout      = 10
  }
  ...
  startup_script = templatefile("${path.module}/tpl/coder_agent_startup_script.bash", {
    ...
    postgres_envvar_dbname_map = {
      DATABASE_URL      = local.postgres_dev_dbname
      TEST_DATABASE_URL = local.postgres_test_dbname
    }
    ...
    git_clone_url              = "git@github.com:usdigitalresponse/cpf-reporter.git"
    git_repo_name              = "cpf-reporter"
    ...
    cpf_reporter_api_url       = local.port_forward_urls["8911"]
    cpf_reporter_website_url   = local.port_forward_urls["8910"]
...
resource "coder_app" "code-server" {
  ...
  icon         = "/icon/code.svg"
  url          = "http://localhost:13337?folder=/home/coder/cpf-reporter"
  subdomain    = false
...
resource "coder_metadata" "workspace_info" {
  ...
  item {
    key   = "website url"
    value = local.port_forward_urls["8910"]
  }

  item {
    key   = "api url"
    value = local.port_forward_domains["8911"]
  }

  item {
    key   = "branch url"
    value = "https://github.com/usdigitalresponse/cpf-reporter/tree/${data.coder_parameter.git_checkout_branch_name.value}"
  }
